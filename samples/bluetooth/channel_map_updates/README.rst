...
See the supported boards listed in 

.. table-from-sample-yaml:: 

The sample currently supports channel map updates to one peripheral device.
To demonstrate this functionality, two development kits are required.  
You can mix different development kits from the list above.  
Each development kit must be connected to a computer with a serial terminal.  

Overview
********

The sample implements a channel map update algorithm that uses connection event reports to collect QoS data.  
...
The algorithm processes QoS data to evaluate and update channel maps. 
...
.. math::
   ...
   r_\text{n} = (1.0 - w_\text{3})\times r_\text{n-1} + w_\text{3} \times (1.0 - (w_\text{1} \times crc_\text{error} + w_\text{2} \times rx_\text{timeout}));
...
* ``crc_error``: crc_errors / packets_sent
* ``rx_timeout``: rx_timeouts / packets_sent
* ``r_(n)``: new calculated rating
* ``r_(n-1)``: the rating from the previous evaluation
...
* Their rating is above a configurable threshold (default: 0.8).
* The number of active channels is greater than the minimum configurable amount (default: 3), to prevent failure of the adaptive channel hopping algorithm.

Key features
============

* Autonomous Operation - Automatically collects QoS data and updates channel maps.  
* Configurable Parameters - Allows adjustment of weights and thresholds to suit different environments.  
* Rating System - Assigns a performance rating to each channel based on CRC errors and RX timeouts.  
...
For physical development kits, build and flash the sample to two development kits, one acting as central, one as peripheral, using ``west build`` and ``west flash`` commands.
...
#. Reset both kits.  
#. On the first development kit, type ``c`` in the terminal to set it as the central (tester).  
#. On the second development kit, type ``p`` in the terminal to set it as the peripheral (peer).  
#. Observe that the kits establish a connection.  
   The central will start gathering QoS statistics.  
#. After the evaluation interval is reached (default: 2000 packets), the algorithm runs automatically. 
#. After a set number of evaluations (default: 5), an evaluation report is printed with the following details:  

   On the Central unit -  
   
   * Channel statistics (packets sent, CRC errors, RX timeouts)  
   * Channel ratings  
   * Channel states (active or inactive)  
   * Updated channel map application 

Testing in simulation:
----------------------

1. Start two instances of the BabbleSim board, one acting as central and the other as peripheral using the following command:  
...
#. In the first terminal, type ``c`` to set it as the central (tester).  
#. In the second terminal, type ``p`` to set it as the peripheral (peer). 
#. Start an interferer (optional), for example, the burst interferer installed with BabbleSim:  
   ...
   This command starts a WLAN-like interference process centered at 2440 MHz with 10 dBm power.

#. Start the physical layer used by the devices:
   ...
   This command starts a physical layer process with the Indoorv1 channel preset and the BLE_simple modem for more realistic packet transfer.  

#. Observe that the devices establish a connection.  
   The central will start gathering QoS statistics. 
#. After the evaluation interval is reached (default: 2000 packets), the algorithm runs automatically.  
#. After a set number of evaluations (default: 5), an evaluation report is printed. 
