...
// Constants
#define CHMAP_RADIO_FREQ_COUNT 81
...
LOG_MODULE_REGISTER(algo, LOG_LEVEL_DBG);
...
void baseline_ch_filter_algo_init(struct chmap_instance *chmap_instance)
{
    ...
    channel_map_filter_set_preferences(chmap_instance,
                                       CHMAP_DEFAULT_DESIRED_ACTIVE_CHANNELS,
...
void print_channel_stats(struct chmap_instance *chmap_instance)
{
    ...
    LOG_INF("Ch | Total_packets_sent  | CRC_Errors | RX_Timeouts | Rating Made | Last Rating | State");
    LOG_INF("---|---------------------|------------|-------------|-------------|-------------|-------");
    ...
    for (int i = 0; i < CHMAP_BT_CONN_CH_COUNT; i++)
    {
        ...
        LOG_INF("%2d | %19d | %10d | %11d | %11.3f | %11.3f | %d",
                i,
                channel_data.total_packets_sent,
                channel_data.total_crc_errors,
                channel_data.total_rx_timeouts,
                (double)rating_used_this_sample,
                (double)rating_used_prev_sample,
                channel_data.state);
    }
}
...
int baseline_ch_filter_algo_evaluate(struct chmap_instance *chmap_instance)
{
    ...
    // Calculate new ratings for all channels, also gives channels cooldown state if rating < thr
    calculate_channel_ratings(chmap_instance);

    // Generate new channel map based on ratings
    generate_channel_map(chmap_instance);

    // Add random channel/s that is done with cooldown to suggested channel map if we are running low on channels
    ...
    printf("Channel map evaluation completed. Active channels: %d\n",
...
static void decrement_cooldown_time_and_update_state(struct chmap_instance *chmap_instance)
{
    for (int i = 0; i < CHMAP_BT_CONN_CH_COUNT; i++)
    {
        ...
        // If decrementation of cooldown lead to cooldown_time_remaining = 0, update state aswell:
        if (chmap_instance->bt_channels[i].cooldown_time_remaining == 0 && chmap_instance->bt_channels[i].state == 2)
        {
...
// Add random disabled channel(s) (state 0, that is: channels where cooldown_time done) to channel map
// to adapt to dynamic enviorments.
static void add_random_channel_to_channel_map(struct chmap_instance *chmap_instance)
{
    ...
    for (int k = 0; k < add_channel_to_channel_map_count; k++)
    {
        ...
        chmap_instance->bt_channels[ch_idx].prev_rating = chmap_instance->bt_channels[ch_idx].rating;
        chmap_instance->active_channel_count++;
                                        ...
                                        const uint8_t min_active_channels)
{
    ...
    if (desired_active_channels < min_active_channels || desired_active_channels > 40 || min_active_channels < 3)
    {
        LOG_WRN("Desired filter preferences are out of bounds, pivoting to default values");
                                            ...
                                            const struct chmap_filter_params *params)
{
    ...
    {
        LOG_WRN("Invalid w_1 value: %f, clamping to 0.0", (double)params->w_1);
        return;
    }
    else if (params->w_2 < 0.0f || params->w_2 > 1.0f)
    {
        LOG_WRN("Invalid w_2 value: %f, clamping to 0.0", (double)params->w_2);
        return;
    }
    else if (params->w_1 + params->w_2 != 1.0f)
    {
        LOG_WRN("Invalid combination of w_1 and w_2. The sum of them should be 1.0 (w1: %f, w2: %f)", (double)params->w_1, (double)params->w_2);
        return;
    }
...
static void set_default_parameters(struct chmap_filter_params *params)
{
    params->w_1 = 0.2f;                   // Weight for CRC errors
    params->w_2 = 0.8f;                   // Weight for RX timeouts
    params->w_3 = 0.850f;                 // Weight for old rating
    params->rating_threshold = 0.7f;      // Disable channels below this rating
...
static void calculate_channel_ratings(struct chmap_instance *chmap_instance)
{
    for (int i = 0; i < CHMAP_BT_CONN_CH_COUNT; i++)
    {
        ...
        if (packets_sent == 0)
        {
            LOG_WRN("Zero packets sent for channel %d", i);
            continue;
        }
        ...
        float temp_rating = (1.0f - chmap_instance->parameters.w_3) * channel->prev_rating +
                            chmap_instance->parameters.w_3 * (1.0f - (chmap_instance->parameters.w_1 * crc_error_rate +
                                                                      chmap_instance->parameters.w_2 * rx_timeout_rate));

        float new_rating = temp_rating;

        // Clamp rating between 0 and 1
        if (new_rating < 0.0f)
        {
            LOG_WRN("Rating below 0.0 (%f), clamping to 0.0", (double)new_rating);
            new_rating = 0.0f;
        }
        else if (new_rating > 1.0f)
        {
            LOG_WRN("Rating above 1.0 (%f), clamping to 1.0", (double)new_rating);
            new_rating = 1.0f;
        }
        ...
        if (channel->rating < chmap_instance->parameters.rating_threshold && channel->state == 1)
        {
            channel->state = 2;
...
static void generate_channel_map(struct chmap_instance *chmap_instance)
{
    ...
    for (int i = 0; i < CHMAP_BT_CONN_CH_COUNT; i++)
    {
        ...
        if (chmap_instance->bt_channels[i].state == 2 || chmap_instance->bt_channels[i].state == 0)
        {
            continue;
        }
        if (chmap_instance->bt_channels[i].rating >= chmap_instance->parameters.rating_threshold)
        {
            int byte_index = i / 8;
            int bit_index = i % 8;
    ...
    {
        printf("Warning: Too few active channels (%d), enabling best channels\n", active_count);

        // Find and enable the best channels, possible that channels that are in cooldown gets enabled
        for (int needed = chmap_instance->min_active_channels - active_count; needed > 0; needed--)
        {
...
void set_suggested_bitmask_to_current_bitmask(struct chmap_instance *chmap_instance)
{
    if (!chmap_instance)
    {
        LOG_ERR("chmap_instance is NULL");
        return;
    }

    uint8_t *new_channel_map = baseline_ch_filter_algo_get_channel_map(chmap_instance);
    if (!new_channel_map)
    {
        LOG_ERR("new_channel_map is NULL");
        return;
    }
