...
#define PERFECT_RATING		 1.0
#define CRC_WEIGHT		 0.15
#define RX_TIMEOUT_WEIGHT	 0.85
#define OLD_RATING_WEIGHT	 0.85
#define RATING_THRESHOLD	 0.8
#define MINIMUM_PACKETS_PER_CHAN 10
...
struct chmap_filter_params {
	...
	uint16_t min_packets_per_channel;
};
...
struct chmap_channel {
	...
	float prev_rating;
	uint8_t state;
	uint8_t cooldown_time_remaining;
...
void channel_map_filter_algo_init(struct chmap_instance *chmap_instance)
{
	...
	for (int i = 0; i < CHMAP_BT_CONN_CH_COUNT; i++) {
		chmap_instance->bt_channels[i].rating = PERFECT_RATING;
		chmap_instance->bt_channels[i].prev_rating = PERFECT_RATING;
...
int channel_map_filter_algo_evaluate(struct chmap_instance *chmap_instance)
{
	...
	/* Add random channels that are done with cooldown to suggested channel map if we are
...
static void add_random_channel_to_channel_map(struct chmap_instance *chmap_instance)
{
	...
	for (int k = 0; k < add_channel_to_channel_map_count; k++) {
		...
		/* update so previous rating is correct */
...
static void set_default_parameters(struct chmap_filter_params *params)
{
	params->w_1 = CRC_WEIGHT;
	params->w_2 = RX_TIMEOUT_WEIGHT;
	params->w_3 = OLD_RATING_WEIGHT;
	params->rating_threshold = RATING_THRESHOLD;
	params->min_packets_per_channel = MINIMUM_PACKETS_PER_CHAN;
}
...
static void calculate_channel_ratings(struct chmap_instance *chmap_instance)
{
	for (int i = 0; i < CHMAP_BT_CONN_CH_COUNT; i++) {
		...
		/* Skip channels where state isn't 1 */
		if (channel->state != 1) {
			continue;
		}
		...
		crc_error_rate = (float)channel->total_crc_errors / packets_sent;
		rx_timeout_rate = (float)channel->total_rx_timeouts / packets_sent;

		new_rating = (1.0f - chmap_instance->parameters.w_3) * channel->prev_rating +
...
static void generate_channel_map(struct chmap_instance *chmap_instance)
{
	...
	if (active_count < chmap_instance->min_active_channels) {
		...
		/* Find and enable the best channels */
